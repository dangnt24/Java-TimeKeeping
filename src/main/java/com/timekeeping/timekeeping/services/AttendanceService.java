...
public class AttendanceService {
    ...
    public AttendanceRecord autoClockIn(int accountID, LocalDateTime clockInTime) {
        ...
        if (lateMinutes >= 15) {
            attendanceRecord.setNotes(String.format("Late by %d minutes", lateMinutes));
        } else {
            attendanceRecord.setNotes("On time");
        }
    ...
    public AttendanceRecord autoClockOut(int accountID, LocalDateTime clockOutTime) {
        ...
        if (actualClockOutTime.isBefore(shiftEndTime)) {
            ...
            attendanceRecord.setNotes(String.format("Left early by %d minutes", earlyDuration.toMinutes()));
        } else {
            // Hoàn thành ca làm
