...
public class HomeController {
    ...
    public String viewNotifications(@CookieValue(value = "ACCOUNT-ID", defaultValue = "0") int accountID,
        ...
        if (activityNotification.isPresent()) {
            ...
            model.addAttribute("activityNotifications", activityNotificationService.getAllNotifications());
            model.addAttribute("activityUnreadNotifications", activityNotificationService.getUnreadNotifications());
            Participation participationInstance = participationService.find(id, accountID);
            if (participationInstance != null) {
                model.addAttribute("isConfirmStatus", participationInstance.getStatus() == ParticipationStatus.JOINED || participationInstance.getStatus() == ParticipationStatus.REASON_APCEPTED);
            }
            model.addAttribute("isConfirm", participationInstance);
    ...
    @GetMapping("/notifications/participate")
    ...
    @PostMapping("/notifications/participate")
    public String apcetParticipate(@RequestParam("id") int id,
                                   @RequestParam("accountID") int accountID,
